import type { AirportConfig, Flight, OperationType, ScheduledOperation } from "./types.js";
...
export interface ScheduleResult {
  ...
  runwayUsage: Map<number, ScheduledOperation[]>;
  summary: ScheduleSummary;
}

const PRIORITIES = { high: 0, medium: 1, low: 2 } as const;

export function separation(previous: OperationType, current: OperationType, config: AirportConfig): number {
...
function emptyRunways(config: AirportConfig): Map<number, ScheduledOperation[]> {
  return new Map(Array.from({ length: config.runwayCount }, (_, index) => [index + 1, []]));
}

function suitableRunways(flight: Flight, runwayUsage: Map<number, ScheduledOperation[]>, config: AirportConfig): number[] {
...
function resourceSlot(resources: number[], earliest: number): { resource: number; start: number } {
  let resource = 0;
...
function runwaySlot(flight: Flight, earliest: number, runwayUsage: Map<number, ScheduledOperation[]>, config: AirportConfig) {
...
function hasDependencyCycle(number: string, flights: Map<string, Flight>, visiting = new Set<string>(), visited = new Set<string>()): boolean {
...
function blockFlight(flight: Flight, reason_code: string, reason: string): void {
  flight.status = "unscheduled";
  flight.reason_code = reason_code;
  flight.reason = reason;
}

export function generateSchedule(state: State, config: AirportConfig): ScheduleResult {
  ...
  while (pending.size) {
    ...
    flight.schedule = {
      runway: runway.runway, gate: gate.resource + 1, ground_crew: crew.resource + 1,
      operation_time: runway.operation_time, gate_window: [serviceStart, serviceStart + config.gateTurnaround], completed_at: completedAt,
    };
