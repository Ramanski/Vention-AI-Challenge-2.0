...
export interface BottleneckResult {
  critical_dependency_chain: string[];
  total_elapsed_duration_minutes: number;
  chain_start_time_minutes: number | null;
  chain_completion_time_minutes: number | null;
  dependency_buffers_applied_minutes: number;
}
...
export function analyzeBottleneck(state: State): BottleneckResult {
  ...
  const chain = (flight: Flight): string[] => {
    ...
    const dependencies = flight.dependencies
      ...
      .filter((item): item is Flight => item?.status === "scheduled");
